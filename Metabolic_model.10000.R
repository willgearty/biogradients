...
# Loop through percentiles of Eo - for median, 0.5 SD and 1 SD
for(E0.quantile in c(0.1587, 0.3085, 0.5, 0.6915, 0.8413)){ 
  ...
  for (Allen.E.no in 1:100){ # Take into account uncertainty on E parameter in Allen et al 2002 using Gillooly & Allen 2007
    ...
    Allen.E  <- E.vec[Allen.E.no]
      ...
      model.summary <- rbind(model.summary,
                             cbind(x$Temp.allen, Allen.E, x$S, E0.quantile, E0.val.sd))
    }}
  print(E0.quantile)
}

names(model.summary) <- c("Temp.allen", "Allen.E", "log.richness", "Eo.quantile", "Eo.sd")

save(model.summary, file = "Metabolic.model.summary.10000.RData")
