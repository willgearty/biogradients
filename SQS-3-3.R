# SQS modified/fixed by William Gearty (willgearty@gmail.com)
# Modifications commented with "-WG"
# sqs version 3.3 by John Alroy from https://bio.mq.edu.au/~jalroy/SQS-3-3.R
...
sqs<-function(ab,q,trials,method,ignore.singletons,dominant)	{
  # the default output from table() has names, which can break the isTRUE fix below -WG
  ab <- as.numeric(ab)
  # change dim from 0 to 1, use list() instead of c() -WG
  params <- array(data=NA,dim=1,dimnames=list("raw richness"))
	if (missing(trials))	{
		trials <- 100
	}
	if (missing(method))	{
	...
	params["Fisher's alpha"] <- NA # because Fisher's alpha calculation is now below -WG
	...
	# moved this and Fisher's alpha down here so you always calculate params -WG
	if (u == 0)	{
	  # return params instead of a print statement -WG
	  print("Coverage is zero because all taxa are singletons",quote=F)
	  return(params)
	...
	if (method == "rarefaction" && q >= specimens - highest)	{
	  # Added print statement here -WG
	  print("Rarefaction quota is equal to or higher than the specimen count",quote=F)
		return(params)
	}
	...
	if ((q > sum(freq) && method != "rarefaction" && method != "CR") || (q >= sum(ab)))	{
	  # Added print statement here -WG
	  print("Quorum target is higher than estimated coverage",quote=F)
		return(params)
	}
	...
	for (trial in 1:trials)	{
		...
		if (method != "rarefaction" && method != "CR")	{
			...
			#  (an even better algorithm added in version 3.0)
				...
				# added to fix cases where sum(freq) == q and to account for machine error -WG
				if (isTRUE(all.equal(udrawn, q))) break
